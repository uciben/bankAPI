...
public class UserPostGres implements UserDAO {
		...
		try (Connection conn = cu.getConnection()){
			...
			if(rs.next()) {
				...
				String sql2 = "select * from person_account join account on "
						+ "person_account.account_id = account.account_id where person_id = ?";
				PreparedStatement pstmt2 = conn.prepareStatement(sql2);
				pstmt2.setInt(1, rs.getInt("person_id"));
	...
	public Set<User> getUserbyRole(Role r) {
		...
		try (Connection conn = cu.getConnection()){
			...
			if(rs.next()) {
				User person = new User(rs.getInt("person_id"), rs.getString("username"), 
						rs.getString("passwd"), rs.getString("firstname"),
						rs.getString("lastname"), rs.getString("email"), 
						new Role(rs.getInt("role_id"), rs.getString("role_name")));
	...
	private boolean addUserAccounts(User u, Connection conn) throws SQLException {
		...
		String sql = "select * from person_account where person_id = ?";
		...
		while(rs.next()) {
			Integer currentId = rs.getInt("person_id");
			
			if(accountIds.contains(currentId)) {
				accountIds.remove(currentId);
			}
		...
		for(Integer id: accountIds) {
			String sql2 = "insert into person_account values (?, ?)";
		...
		return accountsInDB == u.getAccounts().size();
