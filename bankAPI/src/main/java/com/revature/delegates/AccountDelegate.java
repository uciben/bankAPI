...
public class AccountDelegate implements FrontControllerDelegate{
	...
	private void updateAccount(HttpServletRequest req, HttpServletResponse resp) throws IOException{
	    ...
	    Account updateAccount = (Account) om.readValue(req.getInputStream(), Account.class);


		if(isAdmin(currentUser)) {
			if(updateAccount.getAccountId() > 0) { 
				Account theAccount = accServ.getAccountbyID(updateAccount.getAccountId());
				theAccount.setAccountStatus(updateAccount.getAccountStatus());
				theAccount.setAccountType(updateAccount.getAccountType());
				theAccount.setBalance(updateAccount.getBalance());
